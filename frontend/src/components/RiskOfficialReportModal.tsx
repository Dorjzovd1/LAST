...
import { getRiskReport } from "../lib/riskReport";
...
export default function RiskOfficialReportModal({ finding, onClose }: Props) {
  const report = getRiskReport(finding);
    ...
    <div className="risk-modal-overlay" onClick={onClose} role="presentation">
      ...
      >
        <div className="risk-doc-header">
          ...
          <h2 id="risk-report-title">{report.title}</h2>
          <div className="risk-doc-meta">
            <span>Finding ID: #{finding.id}</span>
            <span>Scan ID: #{finding.scan_id}</span>
            <span className={`badge sev-${finding.severity}`}>{ia.severity_label_mn}</span>
          </div>
        </div>
          ...
          <section className="risk-doc-section">
            ...
            <table className="risk-cia-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td><span className={`badge sev-${ia.confidentiality.level === "high" ? "high" : ia.confidentiality.level === "moderate" ? "medium" : "normal"}`}>{ia.confidentiality.label_mn}</span></td>
                  <td>{ia.confidentiality.rationale}</td>
                </tr>
                <tr>
                  <td><strong>I</strong> — Бүрэн бүтэн байдал (Integrity)</td>
                  <td><span className={`badge sev-${ia.integrity.level === "high" ? "high" : ia.integrity.level === "moderate" ? "medium" : "normal"}`}>{ia.integrity.label_mn}</span></td>
                  <td>{ia.integrity.rationale}</td>
                </tr>
                <tr>
                  <td><strong>A</strong> — Байдал (Availability)</td>
                  <td><span className={`badge sev-${ia.availability.level === "high" ? "high" : ia.availability.level === "moderate" ? "medium" : "normal"}`}>{ia.availability.label_mn}</span></td>
            ...
            <div className="risk-doc-result">
              ...
              <div>
                ...
                <strong className={`sev-text-${finding.severity}`}>{ia.severity_label_mn}</strong>
              </div>
            </div>
          </section>
